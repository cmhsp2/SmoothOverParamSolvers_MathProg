function display_objectives(OBJECTIVES, NAMES, TIMES, COLOR, name,format)
...
figure()
if strcmp(format, 'obj_min') %compare to overall minimum objective value
    obj_best = Inf;
    for k = 1:num_
...
for k = 1:num_
    ...
    if strcmp(NAMES{k},name)
        ...
        sp_ = ceil(length(obj_)/5);
     
        h = semilogy( tGrid(1:sp_:end), obj_(1:sp_:end),'Color', COLOR{k},'LineStyle', 'none' , 'Marker',  all_marks{mod(k,13)+1}, 'Markersize',10  );
        hold on
        h1=semilogy( tGrid, obj_, 'Color', COLOR{k},'DisplayName', NAMES{k} );
...
legend(mh_,NAMES,'fontsize',24, 'location', 'southeast')
% legend(mh_,NAMES,'fontsize',24, 'location', 'southwest')
